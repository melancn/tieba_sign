$.post('user.php',
    ...
    function(data) {
        ...
        for(k in data){
            addsdata(data[k].cid,data[k].uname,data[k].cookie,data[k].status);
        }
        addstable();
        addstable();
        getcron();
...
$('.show table tbody').delegate('.name','change',function(){
	...
	if($(this).parent().parent().find('.name').val()&&$(this).parent().parent().find('.cookie').val()){
		...
		this.disabled=true;
		msgshow(this,'正在提交');
		$.post('user.php',u,function(data) {
            msgshow(th,data.msg);
            th.disabled=false;
            if(data.no==0){
                $(th).parent().parent().data('user',data.data);
                th.nextSibling.disabled=false;
                $(th).next().disabled=false;getcron();
            }},'json')
            .error(function(){th.disabled=false;msgshow(th,'提交失败');});
   }else{
...
function addsdata(data){
	var h = c==0 ? 'cookie失效，请立即修改' : '如果你不想修改BDUSS/cookie请勿动此处';
	$('.show table tbody').append('<tr data-user=\'{"type":"up","cid":"'+data.cid+'"}\'><td><input type="text" class="name" maxlength="45" value="'+data.uname+'"></td><td><input type="text" class="cookie" value="'+h+'"></td><td>等待提交</td><td><button class="sub">提交</button><button class="del">删除</button></td></tr>');
}
function addshow(e){
